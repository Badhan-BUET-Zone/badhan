...
//
// The certificate is set in a Latin-only script face — the one the supplied artwork uses — so a
// name stored in Bangla comes out blank, on screen and, worse, on paper. This capture is what that
// failure actually looks like, so the blog can show it instead of only describing it.
...
describe('docs screenshot — a name stored in Bangla', () => {
  it('captures the empty boxes a Bangla name produces', () => {
    ...
    cy.get('@banglaDonorId').then((donorId) => {
      ...
      hideOverlays();
      cy.wait(1500);
      cy.get('[data-cy="certificateFrame"]').screenshot('certificate-bangla-name');
    });
  });
});
