...
describe('The public registration page', () => {
  // No donor is seeded here any more. A registration code is minted from a hall and a session, so
  // nothing on this journey depends on a particular donor existing — which is the premise of the
  // page: the person filling it in is not in the database.
  ...
  it('asks which hall under an All Halls code, and routes the row by the answer', () => {
    ...
    mintRegistrationTokenViaApi(HALL_ANY).then((token) => {
      visitRegistrationPage(token);
  ...
  it('creates no donor', () => {
    ...
    mintRegistrationTokenViaApi(HALL_SUHRAWARDY).then((token) => {
      ...
      // The submitted phone belongs to nobody: if a donor had been created, looking that pair up
      // would now succeed instead of answering 404.
      cy.request({
        method: 'POST',
