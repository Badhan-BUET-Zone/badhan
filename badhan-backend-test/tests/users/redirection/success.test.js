...
test("POST&PATCH/users/redirection: success", async () => {
    ...
    const redirectionResponse = await operations.authedPost('/users/redirection', {}, signInResponse, postUsersRedirectionSchema);
    await operations.authedDelete('/users/signout', signInResponse);
    await operations.guestPatch('/users/redirection', { token: redirectionResponse.data.token }, patchUsersRedirectionSchema);
});
