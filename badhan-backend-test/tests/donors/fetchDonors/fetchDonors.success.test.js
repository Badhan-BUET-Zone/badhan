...
test("GET/donors: success", async () => {
  ...
  await operations.createPlateletDonation(donorResponse.data.donor._id, 1611100800001, signInResponse);
  await operations.markDonorAsActive(donorResponse.data.donor._id, signInResponse);
  await operations.authedPost("/publicContacts", { donorId: donorResponse.data.donor._id, bloodGroup: 2 }, signInResponse);
