...
test('POST&DELETE/callrecords: success', async () => {
  ...
  const recordCreationResponse = await operations.authedPost(
    '/callrecords',
  ...
  await operations.authedDelete(
    `/callrecords?donorId=${donorResponse.data.donor._id}&callRecordId=${recordCreationResponse.data.callRecord._id}`,
    signInResponse,
