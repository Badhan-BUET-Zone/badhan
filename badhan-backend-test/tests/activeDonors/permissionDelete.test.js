...
test('DELETE /activeDonors/:donorId: forbidden when target donor is in different hall and not availableToAll', async () => {
  ...
  // Expect hall-permission error when attempting to delete active donor for donor from a different hall
  await operations.expectErrorWithToken(
    'delete',
    `/activeDonors/${donorHall2.data.newDonor._id}`,
