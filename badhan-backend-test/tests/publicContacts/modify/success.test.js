...
test('POST&DELETE/publicContacts: success', async () => {
  ...
  const contactCreationResponse = await operations.createPublicContact({
    donorId: donorResponse.data.donor._id,
    bloodGroup: 2,
    signInResponse,
  });
  await operations.getPublicContacts(signInResponse); // validates
  await operations.deletePublicContact({
    donorId: donorResponse.data.donor._id,
    contactId: contactCreationResponse.data.publicContact._id,
    signInResponse,
  });
